...
## Libraries, Functions for Data Transformation & Data Loading
...
test_models <- function(data) {
  ...
  results <- bind_rows(
    mutate(performance::test_performance(models_err), Outcome = "Error"),
    mutate(performance::test_performance(models_rt), Outcome = "RT")
  ) |>
    select(-Model, -log_BF) |>
    datawizard::convert_na_to(select="BF", replacement = 1) |>
    arrange(Outcome, desc(BF))|>
    ...
    row_spec(which(results$Outcome == "Error"), background = "#FFCCCC", color = "black") |>
    row_spec(which(results$Outcome == "RT"), background = "#CCCCFF", color = "black") 
}
